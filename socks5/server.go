...
func NewServer(logger *log.Logger, outgoingIf net.IP) *server {
	return &server{
		readTimeoutSecond:  600,
		writeTimeoutSecond: 30,
		logger:             logger,
		outgoingIf:         outgoingIf,
	}
}
...
func (s *server) Run(ctx context.Context, addr *net.TCPAddr) error {
	var err error
...
func connHandle(ctx context.Context, conn *net.TCPConn, s *server) {
	r := &requestList{
		prev: nil, next: nil,
		data: Request{
			ClientConn: conn,
